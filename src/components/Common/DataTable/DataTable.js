...
class DataTable extends React.Component {
	...
	returnItemsPerPage = () => {
		this.props.getItemsPerPage(this.state.itemsPerPage);
	};
	...
	changeItemsPerPage = e => {
		...
		this.setState({ itemsPerPage }, () => {
			this.paginateList(this.state.pageNumber);
			this.props.getItemsPerPage(itemsPerPage);
		});
	};
	...
	search = e => {
		...
		const filteredResults = this.state.list.filter(listItem => {
			let isMatch = '';
			let isNameMatch = '';

			for (let prop in listItem) {
				if (listItem[prop] !== null && listItem[prop] !== undefined)
					isMatch = listItem[prop]
						.toString()
						.toLowerCase()
	...
	actionAll = () => {
		const actionComplete = this.props.actionAllFunction(this.state.checkedRows);
		if (actionComplete) this.setState({ checkedRows: [] });
	};
